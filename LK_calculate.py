...
def calculate_flow(videoName):
    ...
    feature_params = dict(maxCorners=100, qualityLevel=0.1, minDistance=20, blockSize=5)  # 特征点检测参数
    # Lucas-Kanade光流法参数
    lk_params = dict(winSize=(15, 15), maxLevel=2,
                     criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 10, 0.03))
    color = np.random.randint(0, 255, (100, 3))  # 随机生成100种颜色用于绘制光流轨迹
    global p0, initial_positions  # 使用全局变量p0和初始位置
    ret, old = cap.read()  # 读取视频的第一帧
    old_gray = cv2.cvtColor(old, cv2.COLOR_BGR2GRAY)  # 将第一帧转换为灰度图
    p0 = cv2.goodFeaturesToTrack(old_gray, mask=None, **feature_params)  # 使用Shi-Tomasi算法检测特征点
    initial_positions = p0.copy()  # 保存初始特征点位置用于计算位移
    ...
    try:
        ...
        while True:
            ...
            good_new = p1[st == 1]  # 选取跟踪成功的新特征点
            good_initial = initial_positions[st == 1]  # 对应的初始特征点
            ...
            # 绘制光流轨迹
            for i, (new, initial) in enumerate(zip(good_new, good_initial)):
                a, b = new.ravel()  # 新特征点的坐标
                c, d = initial.ravel()  # 对应的初始特征点的坐标
                res1 = float(a - c)  # 计算x方向的位移
                res2 = float(b - d)  # 计算y方向的位移

                l1.append(res1)  # 将x方向位移存储到列表中
                l2.append(res2)  # 将y方向位移存储到列表中
                # 在out图像上绘制从初始位置到当前特征点位置的光流线
                cv2.line(out, (int(a), int(b)), (int(c), int(d)), color[i].tolist(), 3)
                cv2.circle(out, (int(a), int(b)), 3, (255, 0, 0), -1)  # 绘制新的特征点
                cv2.circle(out, (int(c), int(d)), 3, (255, 222, 173), -1)  # 绘制初始特征点
                # 显示编号
                cv2.putText(out, str(i + 1), (int(a), int(b)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
